...
$IacRoot = Join-Path $RepoRoot "infra/external-validation/vultr-tokyo-vm"
$TempRoot = Join-Path ([IO.Path]::GetTempPath()) ("routemind-r4-vm-gate-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Invoke-Checked "python" @("scripts/r4_vm_external_validation.py")

    if (-not (Get-Command terraform -ErrorAction SilentlyContinue)) {
        throw "Terraform is required for the VM external-validation offline gate"
    }
    ...
    Copy-Item -LiteralPath (Join-Path $IacRoot "versions.tf") -Destination $TerraformRoot
    Copy-Item -LiteralPath (Join-Path $IacRoot "variables.tf") -Destination $TerraformRoot
    Copy-Item -LiteralPath (Join-Path $IacRoot "main.tf") -Destination $TerraformRoot
    Copy-Item -LiteralPath (Join-Path $IacRoot "outputs.tf") -Destination $TerraformRoot
    Copy-Item -LiteralPath (Join-Path $IacRoot "cloud-init.yaml.tftpl") -Destination $TerraformRoot
    ...
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        throw "Docker Compose is required for the VM external-validation offline gate"
    }
    ...
    try {
        ...
        Invoke-Checked "docker" @("compose", "-f", (Join-Path $IacRoot "routemind-compose.yaml"), "config", "--quiet")
    }
    finally {
    ...
    Write-Host "PASS: RouteMind Tokyo VM external-validation offline gate"
}
finally {
