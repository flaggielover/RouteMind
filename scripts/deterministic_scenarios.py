...
def load_catalog(path: Path = CATALOG_PATH) -> dict[str, Any]:
    ...
    if (
        ...
        or tuple(item.get("id") for item in records) != EXPECTED_IDS
    ):
        raise ValueError(
            "scenario catalog must contain the frozen scenario IDs in order"
        )
    for record in records:
        ...
        if (
            not isinstance(record.get("demand_count"), int)
            or record["demand_count"] < 1
        ):
            raise ValueError(f"invalid demand_count for {record.get('id')}")
        if len(record.get("demand_ticks", ())) != record["demand_count"]:
            raise ValueError(
                f"demand_ticks does not match demand_count for {record.get('id')}"
            )
        if (
            not isinstance(record.get("courier_count"), int)
            or record["courier_count"] < 1
        ):
...
def build_manifest(record: dict[str, Any], seed: int) -> Any:
    ...
    demands = tuple(
        DemandEvent(
            ...
            order_profile="delayed-preparation"
            if scenario_id == "MERCHANT_DELAY"
            else "standard",
        )
...
def _provider(record: dict[str, Any]) -> Any:
    ...
    if record["provider_mode"] == "fallback":
        ...
        return FallbackTravelTimeProvider(
            _FailingTravelProvider(), local, timeout_seconds=0.1
        )
    return local


def run_scenario(record: dict[str, Any], seed: int) -> dict[str, Any]:
    ...
    if first.replay_digest != second.replay_digest:
        raise AssertionError(
            f"non-deterministic replay digest for {manifest.scenario_id}"
        )
    result = {
        ...
        "assigned_count": sum(
            decision.courier_id is not None for decision in first.decisions
        ),
        "unassigned_count": sum(
            decision.courier_id is None for decision in first.decisions
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--list", action="store_true", help="list the frozen scenario catalog"
    )
    ...
    results = [run_scenario(record, args.seed) for record in records]
    print(
        json.dumps(
