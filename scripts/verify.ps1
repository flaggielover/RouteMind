...
try {
    ...
    python scripts/r4_vm_ssh_readiness_controller_test.py
    if ($LASTEXITCODE -ne 0) {
        throw "Vultr Tokyo VM SSH-readiness execution-controller tests failed"
    }
    ...
    python scripts/r4_vke_connectivity_artifacts_test.py
    if ($LASTEXITCODE -ne 0) {
        throw "VKE diagnostic independent-artifact self-tests failed"
    }
    ...
    python scripts/r4_gmail_single_send_v2_contract.py
    if ($LASTEXITCODE -ne 0) {
        throw "Gmail V2 exactly-one synthetic send contract validation failed"
    }

    python scripts/r4_gmail_single_send_v2_contract_test.py
    if ($LASTEXITCODE -ne 0) {
    ...
    python scripts/r4_gmail_token_refresh_recovery_contract_test.py
    if ($LASTEXITCODE -ne 0) {
        throw "Gmail token refresh recovery contract self-tests failed"
    }
    ...
    python scripts/r4_gmail_oauth_remote_forward_contract_test.py
    if ($LASTEXITCODE -ne 0) {
        throw "Gmail OAuth remote-forward contract self-tests failed"
    }

    python scripts/gmail_oauth_remote_forward_test.py
    if ($LASTEXITCODE -ne 0) {
