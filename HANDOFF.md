...
Task Status: R4-403 passed as implementation `534b9efc8a1ae303bcc26f11207adf47d61efbe6` in all five jobs of GitHub Actions run `32814670130`. R4-404 remains `in_progress`. Run `32818303442` exposed that POSIX `mvnw` was sent to `xdg-open`; run `32818849130` then truly ran 102 Java tests and exposed 15 Linux-order H2 errors after the tenant suite dirtied a shared context. Explicit Bash launchers, a regression gate, and an isolated tenant-test database are implemented. The forced Tenant-before-Business path passes 20/20; full local, clean replacement CI, and artifact validation are pending. The validated local SBOM still has 527 components and explicitly unsigned provenance across Maven, PyPI, npm, and three live OCI manifests.
