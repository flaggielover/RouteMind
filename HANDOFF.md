...
Task Status: R4-405 is `LOCAL_AND_CI_VALIDATED / TARGET_PENDING` and R4-406 is `LOCAL_CI_DRILL_VALIDATED / TARGET_PENDING`. External preparation is locally complete under contract `3e320b5b...1a47d`: self-hosted SigNoz, exact Vultr/IaC inventory, mTLS, actual RouteMind synthetic workload, secrets, retention, cost, failure/recovery, evidence assembly, and teardown are frozen. Authenticated read-only provider preflight passed; no resource mutation, spend, target deployment, telemetry, recovery, or cleanup validation occurred.

Next: cross-volume path remediation `2efb0f6` passed all five jobs in real Actions run `32928947867`; stop at one final Human Gate. Required local values currently pass read-only preflight and must remain securely configured; never put secret values in chat or Git. Final approval must bind contract digest, USD 15 ceiling, eight-hour maximum, and exact inventory. Then run `scripts/r4_external_validation.ps1 -Action Full -AcknowledgeExternalExecution`, validate the final report, commit sanitized evidence, push, observe CI, and recompute R4-407/408/409. R4-410/R4-422 remain independent blocked Human Gates; R4-437 is inactive. Keep `.codex-tmp/` untouched and untracked.

External preparation evidence: `docs/adr/0035-vultr-tokyo-external-validation-backend.md`, `docs/runbooks/R4_VULTR_TOKYO_EXTERNAL_VALIDATION.md`, and `evidence/gates/R4-405/telemetry-export.md`. Terraform `1.9.8` validated provider `2.32.0`; Helm `3.18.6` linted/rendered pinned SigNoz chart `0.138.0` (SHA-256 `b180a601...d7418`) to 32 objects with zero load balancers. Local gates passed 8 preparation mutations, 3 evidence-assembly tests, 9 telemetry mutations, 5 DR contract tests, Java 113/113, Python 925/925 at 95.09%, Web 104/104 plus build, Playwright 34/2, focused resilience 16 Java / 2 Python, security, graph/mirror, and controls. The isolated local DR runtime stopped at an unresponsive Docker Desktop `docker version` before creating resources, so the independent Actions recovery job remains mandatory. R3-325 was not rerun and remains exactly `E-PASS / X-PASS / S-FAIL / C-NO-CLAIM`.

R4-405 evidence: implementation `49680bd` with its then-current digests passed all five Actions jobs in run `32852309878`; preparation revision `10ec537` passed all five jobs in run `32920903229`; cross-volume path remediation `2efb0f6` passed all five jobs in run `32928947867`. The prepared mTLS contract digest is `f063de18402561caf2fa676a152173395183fc0fe660eebe7b3fe185f32b1d5f`; Collector digest is `e1cf3579e04a22cb5210f3f318a123758b4c2b97b7039076507eb27218a36fa7`; external preparation digest is `3e320b5b68924bc1a6843f03b0e421116093fb19cf968a649086028d5c71a47d`. Authenticated catalog-only GETs passed without retained secret values. No resource creation, mutation, spend, running collector, target telemetry, currency cost, experiment, or R3-325 rerun occurred.
