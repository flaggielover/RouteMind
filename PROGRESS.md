...
Current Gate: R4-404 implementation is locally complete and `CI_REMEDIATION`. Initial Actions run 32818303442 exposed that POSIX `mvnw` was being opened by `xdg-open` instead of executed; explicit Bash launchers and a regression gate are implemented locally. A clean replacement run and artifact remain required.
...
- CI: initial run `32818303442` passed Control/Python/Web/resilience but failed
  Java supply-chain generation because POSIX `mvnw` was silently sent to
  `xdg-open`. The explicit Bash launcher fix is locally gated; replacement CI
  is pending. R4-403 `534b9ef` remains green in run `32814670130`.
