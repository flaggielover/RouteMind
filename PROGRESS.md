...
Current Gate: the VKE lane is permanently frozen `EXTERNAL_VKE_VALIDATION = INCONCLUSIVE`; R4-405/R4-406 remain `TARGET_PENDING`, with `NO_TARGET_CLAIM` and `NO_ROOT_CAUSE_CLAIM`. VM v2 contract `b1cf89b...a05b` was consumed once under execution `r4-vm-v2-20260827t051846z-7c7bd60337`; both active hosts closed before an SSH banner and one reboot was unchanged. A new local-only one-VM SSH-readiness diagnostic contract `2ba069c9...fbb7` now isolates TCP, banner, KEX, strict host key, auth, cloud-init, and bootstrap stages. It is unapproved and unexecuted. No new Vultr resource or charge exists.
...
Regression: PASS locally for 21 SSH artifact/protocol/state/fault tests, seven diagnostic-contract mutations, five exact plan mutations, PowerShell case-unambiguous JSON and external-key checks, Terraform 1.9.8/provider 2.32.0 `fmt/init/validate`, a real unapplied `3 create / 0 change / 0 destroy` diagnostic plan, and the full repository controls. Temporary plans and provider cache were deleted. Real CI remains required before the preparation checkpoint closes. R3-325 remains frozen at E-PASS / X-PASS / S-FAIL / C-NO-CLAIM and was not rerun.

Progress Capsule: 165/196 passed; Round 4 is 9/38. `Next eligible: NONE`. VKE is frozen inconclusive with no v4. VM v1/v2 digests are consumed; v2 cost USD 0.246, cumulative conservative external cost USD 11.246, teardown complete, retained resources zero. R4-405/R4-406 stay target-pending/unknown. SSH-readiness contract `2ba069c9...fbb7` proposes one `vc2-1c-1gb`, one firewall, one operator `/32`, 60 minutes, USD 1 ceiling, exact teardown; it awaits a fresh Human Gate. R4-410 `7f71f0...75d7` and R4-422 `0cc9bc...4ffb` remain independent unapproved gates.
...
Blocked: R4-405/R4-406 still require real RouteMind telemetry and DR target evidence. VM v2 cannot be retried. The new diagnostic can only localize SSH readiness and cannot promote either task; it requires approval of fresh SHA-256 `2ba069c9886c69f1b38a22740c6c2367bd21a2bd129e8ff6c8148f336a46fbb7` before one paid VM may be created. Existing VPCs remain `UNKNOWN / NOT_SAFE_TO_REUSE`; VKE-specific properties remain `DEFERRED_VKE`. R4-410 and R4-422 retain their independent provider/recipient gates. R4-437 remains inactive.

Human Action Required: approve exactly one independent gate. The Tokyo VM gate is the one-VM SSH-readiness diagnostic contract SHA-256 `2ba069c9886c69f1b38a22740c6c2367bd21a2bd129e8ff6c8148f336a46fbb7`, `nrt`, 60 minutes, USD 1 incremental ceiling, and exact teardown. R4-410 may instead ratify `7f71f0...75d7`; R4-422 may instead ratify `0cc9bc...4ffb`. These gates are independent and none is implicitly approved by another.

Next Candidates: none safely executable without a new Human Gate. The dependency-only candidate R4-437 remains inactive. VKE v4 is prohibited; SSH-readiness paid execution, R4-410 provider ratification, and R4-422 provider/channel ratification each require independent approval.
