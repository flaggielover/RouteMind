...
Current Gate: R3-346 closed `INSUFFICIENT_DATA/C-NO-CLAIM` after implementation `43e3549` passed all five jobs in Actions run `32784278395`. R3-347 is active and must audit Decision X-Ray replay, perturbation, delta, minimality, and lineage support before any counterfactual experiment.
...
Next Candidates: Audit R3-347 support from the frozen R3-350 corpus and RM-226 Decision X-Ray implementation. Require original/new decision linkage, perturbation provenance, objective/risk deltas, and minimality verification; label any output model/system replay and make no causal claim. R3-313 remains optional and non-blocking.
