...
Current Gate: second Vultr Tokyo execution `r4-ext-20260826t054111z-ea80181368` failed closed before Kubernetes mutation because the provider-created VKE firewall group had zero rules. Exact teardown plus credentialed VKE/recovery/recovery-firewall/VKE-firewall 404 and zero-label checks passed. Its conservative bound is USD 0.24 and the two-attempt aggregate is USD 0.48. Remediation adds one Terraform-managed operator `/32` TCP 6443 rule and VKE firewall cleanup evidence. The new exact contract SHA-256 is `c2a16951...f4e2df`; no new paid execution is authorized before approval.

CI: R4-405 implementation `49680bd` passed all five jobs in run `32852309878`; preparation revision `10ec537` passed all five jobs in run `32920903229`; cross-volume path remediation `2efb0f6` passed all five jobs in run `32928947867`; first-attempt remediation `fb6adcd` passed all five jobs in run `32934187355`; VKE firewall remediation `160f670` passed all five jobs in run `32937109761`, including Linux offline Terraform/Helm and recovery gates. Current mTLS/collector digests remain `f063de18...1d5f` / `e1cf3579...6fa7`. Recovery artifacts remain separate and do not qualify the target.
...
Progress Capsule: `resume.ps1` reports `Current: NONE`, 165/196 passed, `Next eligible: NONE`, and the four truthful external gates. VKE firewall remediation `160f670` is CI-green in Actions run `32937109761`. R4-410/R4-422 are blocked on their independent travel/notification approvals; conditional R4-437 is inactive and no longer appears eligible.
...
Blocked: R4-405/R4-406 require approval of remediated contract SHA-256 `c2a16951...f4e2df` under the aggregate USD 15/eight-hour boundary, then matching target evidence. R4-410 and R4-422 retain separate provider/notification Human Gates; R4-437 remains conditional and inactive.

Human Action Required: after remediation CI is green, approve exact contract SHA-256 `c2a1695104ba7297b51b1c949fa689a4efeb5974dcf1a2122c12f91a57f4e2df`, keeping the aggregate USD 15 ceiling, eight-hour maximum, and mandatory teardown. Existing secrets stay configured outside Git and must not be sent in chat.

Next Candidates: none before the remediated external Human Gate. After approval, rerun R4-405/R4-406 with a fresh execution ID and continue to R4-407 only if target evidence passes. R4-410/R4-422 remain independent blocked gates.
