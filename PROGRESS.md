...
Current Task: NONE - VKE TLS EOF DIAGNOSTIC HUMAN GATE for R4-405/R4-406

Last Completed: R4-401 - Select a deployment target and freeze SLO and failure-domain assumptions

Current Gate: third Vultr Tokyo execution `r4-ext-20260826t063255z-18f9f4f51b` created exactly five approved resources but the VKE control plane still closed TLS before handshake despite the operator `/32` TCP 6443 rule. No Kubernetes mutation occurred. Full teardown destroyed all five resources; credentialed VKE/recovery/recovery-firewall/VKE-firewall 404 and zero-label checks passed. The authenticated eight-hour quote bound is USD 3.92 and the three-attempt aggregate conservative bound is USD 4.40. The consumed contract SHA-256 remains `c2a16951...f4e2df`; R4-405/R4-406 remain target-pending. Read-only diagnosis is recorded in `docs/closure/r4/VKE_TLS_EOF_DIAGNOSTIC.md`; the prepared minimal diagnostic contract digest is `30c9580e...4a426` and is not authorized.
...
Progress Capsule: `resume.ps1` reports `Current: NONE`, 165/196 passed, `Next eligible: NONE`, and the four truthful external gates. The third target attempt failed closed after provider API TLS EOF and completed zero-resource cleanup; evidence checkpoint `fd94ce2` is CI-green in Actions run `32945284919`. The new VKE diagnostic tool/contract are CI-green in Actions run `32948600781` and await a distinct Human Gate; R4-410/R4-422 are blocked on their independent travel/notification approvals; conditional R4-437 is inactive and no longer appears eligible.
...
Blocked: R4-405/R4-406 require the VKE TLS EOF differential diagnosis and then credentialed target evidence; the current approved digest produced no target evidence. The prepared minimal diagnostic contract digest `30c9580e...4a426` requires a new Human Gate and must not be treated as approved. R4-410 and R4-422 retain separate provider/notification Human Gates; R4-437 remains conditional and inactive.

Human Action Required: review `docs/closure/r4/VKE_TLS_EOF_DIAGNOSTIC.md`. If the two-observer, two-hour diagnostic is approved, authorize exact digest `30c9580eb2fe43de1306b299a73c4a1c5d0f286ac7bef4be0c3d0f4b7994a426`, USD 5.00 incremental ceiling, and mandatory teardown. Existing secrets stay configured outside Git and must not be sent in chat.
